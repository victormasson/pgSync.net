...
public class DataSource
{
    ...
    public List<Trigger> Triggers(Table table)
    {
        string query = @"
                SELECT
                    tgname AS name,
        ...
        return ExecuteList(query, new NpgsqlParameter("$1", table.FullName), reader => new Trigger
        {
            Name = reader.GetString(0),
            Internal = reader.GetBoolean(1),
            Enabled = reader.GetBoolean(2),
            Integrity = reader.GetBoolean(3)
        });
    }
    ...
    private string QuoteIdentFull(Ident ident)
    {
        if (ident is Table || ident is Sequence)
        {
            return $"{QuoteIdent(ident.Schema)}.{QuoteIdent(ident.Name)}";
        }
        else
        {
    ...
    {
        return NpgsqlConnection.GlobalTypeMapper
            .MapToPgTypeName(value)
            .ToString();
    }

    private string DedupLocalHost(string value)
    {
